...
static NNUE: NnueNet = unsafe { std::mem::transmute(*include_bytes!("../resources/net.bin")) };
...
fn nnue_eval(board: &Board) -> i32 {
    ...
    let wksq = match king_square(board, Color::White) {
        Some(s) => s.to_index(),
        None => return 0,
    };
    let bksq = match king_square(board, Color::Black) {
        Some(s) => s.to_index(),
        None => return 0,
    };
    ...
    let (boys, opps) = if stm == Color::White {
        (&white_acc, &black_acc)
    } else {
        (&black_acc, &white_acc)
    };
